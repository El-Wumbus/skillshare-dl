...
class Downloader(object):
    ...
    def download_course_by_url(self, url, boolSubtitle, all_subs, boolResources):
        ...
        self.download_course_by_class_id(
            class_id, boolSubtitle, all_subs, boolResources)
                ...
                if 'video_hashed_id' in s and s['video_hashed_id']:
                    ...
                    self.download_video(
                        fpath='{base_path}/{session}.mp4'.format(
                            base_path=self.base_path, session=file_name),
                        video_id=video_id,
                        tPath='{t_path}/{session}.mp4'.format(
                            t_path=temp_base_path, session=file_name),
    ...
    def downloadToStorage(self, url, path, tPath):
        ...
        with open(tPath, 'wb') as (f):
            ...
            file_size = 'File Size : ' + \
                str(round(int(total_length) / 1e+6)) + ' mb'
